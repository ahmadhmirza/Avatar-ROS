...
# Add zero matrix image to the input arraay to make them the same size
# Param     : array containing the image sequence
# Param-2   : integer - final size of the array
#return     : Padded image array
def addPadding(imageArray,paddingSize):
    size = 224,224
    placeHolderMatrix = np.zeros(size, dtype=np.uint8)
    #placeHolderImage = cv2.fromarray(placeHolderMatrix)
    if len(imageArray)<paddingSize:
        for i in range(0,(paddingSize-len(imageArray))):
            imageArray.append(placeHolderMatrix)
    return imageArray

def flattenList(batchList):
    flat_list = []
...
for folder in range(1,p01_dataEntries+1):
    ...
    for file in glob.glob(dataPath):
        ...
        #print(height,width)
        batchList.append(img)
    
    ###### All content in the folder has been read        #####
    ###### Corresponding to one sequence         ##############
    ###### batchList includes all the images in a sequence#####
    
    flatBatchList = flattenList(batchList)
    #npBatchList = np.array(batchList)
    
    X.append(flatBatchList)
